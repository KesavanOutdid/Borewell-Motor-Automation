...
import useLiveTelemetry from "../../../hooks/Admin/ManageDevice/useLiveTelemetry";
import useLiveStatus from "../../../hooks/Admin/ManageDevice/useLiveStatus";
import useLiveAlert from "../../../hooks/Admin/ManageDevice/useLiveAlert";
import useLiveBoot from "../../../hooks/Admin/ManageDevice/useLiveBoot";
import useLiveHeartbeat from "../../../hooks/Admin/ManageDevice/useLiveHeartbeat";
import useDeviceSchedules from "../../../hooks/Admin/ManageDevice/useDeviceSchedules";
...
const Dashboard = ({ userInfo, handleLogout }) => {
    ...
    const { status } = useLiveStatus(selectedAssignedDevice?.serial_number);
                            ...
                            <div className="card h-100">
                                ...
                                <div className="card-body p-3">
                                    <div className="timeline timeline-one-side">
                                        ...
                                        <div className="timeline-block">
                                            <span className="timeline-step">
                                                <i className="fas fa-calendar-alt opacity-10 text-success text-gradient"></i>
                                            </span>
                                            <div className="timeline-content">
                                                <h6 className="text-dark text-sm font-weight-bold mb-0">Device Schedules</h6>
                                            </div>
                                        </div>
                                        
                                        <div className="mt-4" style={{ maxHeight: '275px', overflowY: 'auto', paddingRight: '8px' }}>
                                            {loadingSchedules ? (
                                                <div className="text-center py-3">
                                                    <div className="spinner-border spinner-border-sm text-success" role="status">
                                                        <span className="visually-hidden">Loading...</span>
                                                    </div>
                                                    <p className="text-xs mt-2">Loading schedules...</p>
                                                </div>
                                            ) : schedules && schedules.length > 0 ? (
                                                schedules.map((schedule, index) => {
                                                    const getStatusStyles = (status) => {
                                                        switch(status) {
                                                            case 'completed': return { color: 'success', icon: 'fa-check-circle' };
                                                            case 'cancelled': return { color: 'danger', icon: 'fa-times-circle' };
                                                            case 'started': return { color: 'info', icon: 'fa-play-circle' };
                                                            case 'pending': return { color: 'warning', icon: 'fa-clock' };
                                                            default: return { color: 'secondary', icon: 'fa-calendar' };
                                                        }
                                                    };
                                                    const { color: statusColor, icon: statusIcon } = getStatusStyles(schedule.status);

                                                    return (
                                                        <div key={index} className="timeline-block mb-3 pb-3">
                                                            <span className="timeline-step">
                                                                <i className={`fas ${statusIcon} text-${statusColor}`}></i>
                                                            </span>
                                                            <div className="timeline-content">
                                                                <div className="d-flex justify-content-between align-items-center mb-2">
                                                                    <span className={`badge badge-sm bg-gradient-${statusColor}`}>
                                                                        {schedule.status.toUpperCase()}
                                                                    </span>
                                                                    <span className="text-secondary text-xxs font-weight-bold">
                                                                        {formatDateTime(schedule.created_at)}
                                                                    </span>
                                                                </div>
                                                                
                                                                <div className="mb-2">
                                                                    <p className="text-dark text-xs font-weight-bold mb-0">
                                                                        <i className="fas fa-user text-xxs me-1 text-primary"></i> 
                                                                        User: <span className="text-secondary font-weight-normal">{schedule.user_name || "System"}</span>
                                                                    </p>
                                                                    {schedule.status === 'cancelled' && schedule.cancelled_by && (
                                                                        <p className="text-dark text-xs font-weight-bold mb-0 mt-1">
                                                                            <i className="fas fa-user-slash text-xxs me-1 text-danger"></i>
                                                                            Cancelled by: <span className="text-danger font-weight-normal">{schedule.cancelled_by}</span>
                                                                        </p>
                                                                    )}
                                                                </div>

                                                                <div className="bg-gray-100 p-2 border-radius-lg">
                                                                    <div className="d-flex justify-content-between mb-1">
                                                                        <span className="text-secondary text-xxs font-weight-bold">
                                                                            <i className="fas fa-play text-success me-1"></i> Start
                                                                        </span>
                                                                        <span className="text-dark text-xxs font-weight-bold">
                                                                            {formatDateTime(schedule.start_time)}
                                                                        </span>
                                                                    </div>
                                                                    <div className="d-flex justify-content-between">
                                                                        <span className="text-secondary text-xxs font-weight-bold">
                                                                            <i className="fas fa-stop text-danger me-1"></i> Stop
                                                                        </span>
                                                                        <span className="text-dark text-xxs font-weight-bold">
                                                                            {formatDateTime(schedule.stop_time)}
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                ...
                                                <div className="text-center py-4">
                                                    <i className="fas fa-calendar-times text-secondary opacity-5 mb-2" style={{ fontSize: '2rem' }}></i>
                                                    <p className="text-secondary text-xs">No schedules found for this device</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                            ...
                            <div className="card h-100">
                                ...
                                <div className="card-body px-0 pb-2" style={{ paddingTop: '0' }}>
                                    <div className="table-responsive">
                                        <div style={{ width: '100%', height: '600px', position: 'relative', padding: '20px' }}>
